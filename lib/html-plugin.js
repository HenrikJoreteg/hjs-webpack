...
function defaultHtml (incomingData) {
  // <chcokr>
  var isDev = incomingData.isDev;
  var reactString = incomingData.reactString;
  var stilrStylesheet = incomingData.stilrStylesheet;
  // </chcokr>
  ...
  // <chcokr>
  if (!data.isDev) {
    add('<style>' + stilrStylesheet + '</style>');
  }
  // </chcokr>
  if (data.head) {
    add(data.head)
...
HJSPlugin.prototype.apply = function (compiler) {
  ...
  compiler.plugin('emit', function (compiler, callback) {
    ...
    if (!isDev) {
      ...
      var asset = compiler.assets[context.main];
      ...
      context.reactString =
        React.renderToString(React.createElement(evalResult.Component));
      context.stilrStylesheet =
